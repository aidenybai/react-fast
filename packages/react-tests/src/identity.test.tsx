...
describe("ReactIdentity", () => {
  it("should allow key property to express identity", async () => {
    ...
    const Component = (props: { swap?: boolean }) => (
  ...
  it("should use composite identity", async () => {
    ...
    const container = document.createElement("div");
    const root = ReactDOMClient.createRoot(container);
    let node1: HTMLSpanElement | null = null;
    let node2: HTMLSpanElement | null = null;
    await act(async () => {
      root.render(
        <Wrapper key="wrap1">
          <span ref={(element) => (node1 = element)} />
        </Wrapper>,
      );
    });
    await act(async () => {
      root.render(
        <Wrapper key="wrap2">
          <span ref={(element) => (node2 = element)} />
        </Wrapper>,
      );
    });
  ...
  async function renderAComponentWithKeyIntoContainer(key: string, root: ReactDOMClient.Root) {
  ...
  it("should not allow scripts in keys to execute", async () => {
    const h4x0rKey = "\"><script>window['YOUVEBEENH4X0RED']=true;</script><div id=\"";
    ...
    expect((window as unknown as Record<string, unknown>).YOUVEBEENH4X0RED).toBe(undefined);
  });
  ...
  it("should let restructured components retain their uniqueness", async () => {
    ...
    class TestComponent extends React.Component<{ children: React.ReactNode }> {
      render() {
        return (
          <div>
            {instance2}
            {(this.props.children as React.ReactNode[])[0]}
            {(this.props.children as React.ReactNode[])[1]}
          </div>
        );
      }
    }
  ...
  it("should retain key during updates in composite components", async () => {
    ...
    const container = document.createElement("div");
    const root = ReactDOMClient.createRoot(container);
    const wrappedRef = React.createRef<TestContainer>();
    await act(async () => {
      root.render(<TestContainer first={instance0} second={instance1} ref={wrappedRef} />);
    });
  ...
  it("should throw if key is a Temporal-like object", async () => {
    ...
    expect(() => {
      root.render(
        <div>
          <span key={new TemporalLike() as unknown as string} />
        </div>,
      );
